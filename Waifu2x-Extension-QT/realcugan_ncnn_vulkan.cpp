...
int MainWindow::RealCUGAN_NCNN_Vulkan_Image(int rowNum, bool ReProcess_MissingAlphaChannel)
{
    ...
    //============================== 放大 =======================================
    QString Waifu2x_folder_path = Current_Path + "/realcugan-ncnn-vulkan";
    QString program = Waifu2x_folder_path + "/realcugan-ncnn-vulkan.exe";
    //==========
    ...
    for (int retry = 0; retry < (ui->spinBox_retry->value() + ForceRetryCount); retry++)
    {
        ...
        for (int i = 4; i <= ScaleRatio_tmp; i *= 4)
        {
            ...
            args.append(RealCUGAN_NCNN_Vulkan_ReadSettings());
...
int MainWindow::RealCUGAN_NCNN_Vulkan_GIF(int rowNum)
{
    ...
    for (int i = 0; i < Frame_fileName_list.size(); i++)
    {
        ...
        QtConcurrent::run(this, &MainWindow::RealCUGAN_NCNN_Vulkan_GIF_scale, Sub_Thread_info, &Sub_gif_ThreadNumRunning, &Frame_failed);
...
int MainWindow::RealCUGAN_NCNN_Vulkan_GIF_scale(QMap<QString, QString> Sub_Thread_info, int *Sub_gif_ThreadNumRunning, bool *Frame_failed)
{
...
int MainWindow::RealCUGAN_NCNN_Vulkan_Video(int rowNum)
{
    ...
    {
        emit Send_video_write_VideoConfiguration(VideoConfiguration_fullPath, ScaleRatio, 0, CustRes_isEnabled, CustRes_height, CustRes_width, "realcugan-ncnn-vulkan", false, "", "");
    }
    //=======================
    //   检测缓存是否存在
    //=======================
    ...
    for (int i = 0; i < Frame_fileName_list.size(); i++)
    {
        ...
        QString Frame_fileName = Frame_fileName_list.at(i);
        Sub_Thread_info["Frame_fileName"] = Frame_fileName;
        QtConcurrent::run(this, &MainWindow::RealCUGAN_NCNN_Vulkan_Video_scale, Sub_Thread_info, &Sub_video_ThreadNumRunning, &Frame_failed);
...
int MainWindow::RealCUGAN_NCNN_Vulkan_Video_BySegment(int rowNum)
{
    ...
    {
        emit Send_video_write_VideoConfiguration(VideoConfiguration_fullPath, ScaleRatio, 0, CustRes_isEnabled, CustRes_height, CustRes_width, "realcugan-ncnn-vulkan", true, VideoClipsFolderPath, VideoClipsFolderName);
    }
    //=======================
    //   检测缓存是否存在
    //=======================
...
/*
===============================
RealCUGAN_NCNN_Vulkan_Video_scale
负责 视频帧画面放大与后期调整
===============================
*/
int MainWindow::RealCUGAN_NCNN_Vulkan_Video_scale(QMap<QString, QString> Sub_Thread_info, int *Sub_video_ThreadNumRunning, bool *Frame_failed)
{
    QString SplitFramesFolderPath = Sub_Thread_info["SplitFramesFolderPath"];
    QString ScaledFramesFolderPath = Sub_Thread_info["ScaledFramesFolderPath"];
    QString SourceFile_fullPath = Sub_Thread_info["SourceFile_fullPath"];
...
预读取引擎配置
*/
QStringList MainWindow::RealCUGAN_NCNN_Vulkan_PreLoad_Settings()
{
    QStringList RealCUGAN_NCNN_Vulkan_Settings_str;
    if (ui->checkBox_MultiGPU_RealCUGANNcnnVulkan->isChecked() == false)
    {
        //==========单显卡==========
        // GPU ID
        if (ui->comboBox_GPUID_RealCUGANNCNNVulkan->currentText() != "auto")
        {
            RealCUGAN_NCNN_Vulkan_Settings_str.append("-g");
            RealCUGAN_NCNN_Vulkan_Settings_str.append(ui->comboBox_GPUID_RealCUGANNCNNVulkan->currentText());
        }
        // Tile Size
        RealCUGAN_NCNN_Vulkan_Settings_str.append("-t");
        RealCUGAN_NCNN_Vulkan_Settings_str.append(QString::number(ui->spinBox_TileSize_RealCUGANNCNNVulkan->value(), 10));
    }
    // TTA
    if (ui->checkBox_TTA_RealCUGANNCNNVulkan->isChecked())
    {
        RealCUGAN_NCNN_Vulkan_Settings_str.append("-x");
    }
    // Model
    QString Waifu2x_folder_path = Current_Path + "/realcugan-ncnn-vulkan";
    switch (ui->comboBox_Model_RealCUGANNCNNVulkan->currentIndex())
    {
    case 0:
    {
        RealCUGAN_NCNN_Vulkan_Settings_str.append("-m");
        RealCUGAN_NCNN_Vulkan_Settings_str.append(Waifu2x_folder_path + "/models-nose");
        break;
    }
    case 1:
    {
        RealCUGAN_NCNN_Vulkan_Settings_str.append("-m");
        RealCUGAN_NCNN_Vulkan_Settings_str.append(Waifu2x_folder_path + "/models-se");
        break;
    }
    case 2:
    {
        RealCUGAN_NCNN_Vulkan_Settings_str.append("-m");
        RealCUGAN_NCNN_Vulkan_Settings_str.append(Waifu2x_folder_path + "/models-pro");
        break;
    }
    }
    RealCUGAN_NCNN_Vulkan_Settings_str.append("-j");
    RealCUGAN_NCNN_Vulkan_Settings_str.append("1:1:1");
    //=======================================
...
读取配置生成配置QString
*/
QStringList MainWindow::RealCUGAN_NCNN_Vulkan_ReadSettings()
{
    QStringList RealCUGAN_NCNN_Vulkan_Settings_str;
    RealCUGAN_NCNN_Vulkan_Settings_str.append(RealCUGAN_NCNN_Vulkan_PreLoad_Settings_Str);
    if (ui->checkBox_MultiGPU_RealCUGANNcnnVulkan->isChecked())
    {
        //==========多显卡==========
        QMap<QString, QString> GPUInfo = RealCUGANNcnnVulkan_MultiGPU();
        // GPU ID
        RealCUGAN_NCNN_Vulkan_Settings_str.append("-g");
        RealCUGAN_NCNN_Vulkan_Settings_str.append(GPUInfo["ID"]);
        // Tile Size
        RealCUGAN_NCNN_Vulkan_Settings_str.append("-t");
        RealCUGAN_NCNN_Vulkan_Settings_str.append(GPUInfo["TileSize"]);
    }
    //=======================================
    return RealCUGAN_NCNN_Vulkan_Settings_str;
}
...
点击检测gpu按键时:
*/
void MainWindow::on_pushButton_DetectGPU_RealCUGANNCNNVulkan_clicked()
{
    //====
    ui->pushButton_DetectGPU_RealCUGANNCNNVulkan->setText("Detecting, please wait...");
    //====
...
int MainWindow::RealCUGAN_ncnn_vulkan_DetectGPU()
{
    ...
    QString Waifu2x_folder_path = Current_Path + "/realcugan-ncnn-vulkan";
    QString program = Waifu2x_folder_path + "/realcugan-ncnn-vulkan.exe";
    QString model_path = Waifu2x_folder_path + "/models-se";
    //===========
    int GPU_ID = -1;
    //=========
    while (true)
    {
        ...
        QStringList args = {"-i", InputPath, "-o", OutputPath, "-s", "4", "-t", "32", "-m", model_path, "-g", QString::number(GPU_ID, 10)};
        Waifu2x->start(program, args);
        if(waitForProcess(Waifu2x)==TerminatedByFlag){
            delete Waifu2x;
            return 0;
        }
        delete Waifu2x;
...
int MainWindow::RealCUGAN_ncnn_vulkan_DetectGPU_finished()
{
    ...
    //====
    GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.clear();
    ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->clear();
    //===
    ui->comboBox_GPUID_RealCUGANNCNNVulkan->clear();
    ui->comboBox_GPUID_RealCUGANNCNNVulkan->addItem("auto");
    if (!Available_GPUID_RealCUGAN_ncnn_vulkan.isEmpty())
    {
        for (int i = 0; i < Available_GPUID_RealCUGAN_ncnn_vulkan.size(); i++)
        {
            ui->comboBox_GPUID_RealCUGANNCNNVulkan->addItem(Available_GPUID_RealCUGAN_ncnn_vulkan.at(i));
            AddGPU_MultiGPU_RealCUGANNcnnVulkan(Available_GPUID_RealCUGAN_ncnn_vulkan.at(i));
        }
    }
    //====
    ui->pushButton_DetectGPU_RealCUGANNCNNVulkan->setText("Detect available GPU ID");
    //====
    return 0;
}
/*
RealCUGANNcnnVulkan_MultiGPU
向RealCUGAN_NCNN_Vulkan_ReadSettings()提交显卡信息.
通过 轮流提交显卡信息+多线程 一起实现多显卡
*/
QMap<QString, QString> MainWindow::RealCUGANNcnnVulkan_MultiGPU()
{
    MultiGPU_QMutex_RealCUGANNcnnVulkan.lock();
    //====
    int MAX_GPU_ID_RealCUGANNcnnVulkan = GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.count() - 1;
    if (GPU_ID_RealCUGANNcnnVulkan_MultiGPU > MAX_GPU_ID_RealCUGANNcnnVulkan)
    {
        GPU_ID_RealCUGANNcnnVulkan_MultiGPU = 0;
    }
    //======
    QMap<QString, QString> GPUInfo;
    do
    {
        GPUInfo = GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.at(GPU_ID_RealCUGANNcnnVulkan_MultiGPU);
        if (GPUInfo["isEnabled"] != "true")
        {
            GPU_ID_RealCUGANNcnnVulkan_MultiGPU++;
            if (GPU_ID_RealCUGANNcnnVulkan_MultiGPU > MAX_GPU_ID_RealCUGANNcnnVulkan)
            {
                GPU_ID_RealCUGANNcnnVulkan_MultiGPU = 0;
            }
    ...
    //======
    MultiGPU_QMutex_RealCUGANNcnnVulkan.unlock();
    return GPUInfo;
}
/*
向多显卡的显卡信息列表中添加显卡信息
*/
void MainWindow::AddGPU_MultiGPU_RealCUGANNcnnVulkan(QString GPUID)
{
    QMap<QString, QString> GPUInfo;
    GPUInfo["ID"] = GPUID;
    GPUInfo["isEnabled"] = "true";
    GPUInfo["TileSize"] = "100";
    GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.append(GPUInfo);
    ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->addItem(GPUID);
    ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->setCurrentIndex(0);
}

void MainWindow::on_checkBox_MultiGPU_RealCUGANNcnnVulkan_stateChanged(int arg1)
{
    if (ui->checkBox_MultiGPU_RealCUGANNcnnVulkan->isChecked())
    {
        ui->comboBox_GPUID_RealCUGANNCNNVulkan->setEnabled(0);
        ui->spinBox_TileSize_RealCUGANNCNNVulkan->setEnabled(0);
        ui->groupBox_GPUSettings_MultiGPU_RealCUGANNcnnVulkan->setEnabled(1);
    }
    else
    {
        ui->comboBox_GPUID_RealCUGANNCNNVulkan->setEnabled(1);
        ui->spinBox_TileSize_RealCUGANNCNNVulkan->setEnabled(1);
        ui->groupBox_GPUSettings_MultiGPU_RealCUGANNcnnVulkan->setEnabled(0);
    }
}

void MainWindow::on_checkBox_MultiGPU_RealCUGANNcnnVulkan_clicked()
{
    if (ui->checkBox_MultiGPU_RealCUGANNcnnVulkan->isChecked())
    {
        if (GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.count() == 0)
        {
            QMessageBox::information(this,"Notification","Please detect available GPU ID before enable Multi-GPU.");
            ui->checkBox_MultiGPU_RealCUGANNcnnVulkan->setChecked(0);
            return;
        }
        if (GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.count() < 2)
        {
...
void MainWindow::on_comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan_currentIndexChanged(int index)
{
    if (ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->count() == 0)
    {
        return;
    }
    QMap<QString, QString> GPUInfo = GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.at(ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->currentIndex());
    ui->checkBox_isEnable_CurrentGPU_MultiGPU_RealCUGANNcnnVulkan->setChecked(GPUInfo["isEnabled"] == "true");
    ui->spinBox_TileSize_CurrentGPU_MultiGPU_RealCUGANNcnnVulkan->setValue(GPUInfo["TileSize"].toInt());
}

void MainWindow::on_checkBox_isEnable_CurrentGPU_MultiGPU_RealCUGANNcnnVulkan_clicked()
{
    QMap<QString, QString> GPUInfo = GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.at(ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->currentIndex());
    if (ui->checkBox_isEnable_CurrentGPU_MultiGPU_RealCUGANNcnnVulkan->isChecked())
    {
        GPUInfo["isEnabled"] = "true";
    }
    else
    ...
    GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.replace(ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->currentIndex(), GPUInfo);
    int enabledGPUs = 0;
    for (int i = 0; i < GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.count(); i++)
    {
        QMap<QString, QString> GPUInfo_tmp = GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.at(i);
    ...
    if (enabledGPUs < 2)
    {
        ...
        GPUInfo["isEnabled"] = "true";
        GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.replace(ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->currentIndex(), GPUInfo);
        ui->checkBox_isEnable_CurrentGPU_MultiGPU_RealCUGANNcnnVulkan->setChecked(1);
        QMessageBox::warning(this,"Warning","At least 2 GPUs need to be enabled !!");
    }
}

void MainWindow::on_spinBox_TileSize_CurrentGPU_MultiGPU_RealCUGANNcnnVulkan_valueChanged(int arg1)
{
    QMap<QString, QString> GPUInfo = GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.at(ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->currentIndex());
    GPUInfo["TileSize"] = QString::number(ui->spinBox_TileSize_CurrentGPU_MultiGPU_RealCUGANNcnnVulkan->value(), 10);
    GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.replace(ui->comboBox_GPUIDs_MultiGPU_RealCUGANNcnnVulkan->currentIndex(), GPUInfo);
}

void MainWindow::on_pushButton_ShowMultiGPUSettings_RealCUGANNcnnVulkan_clicked()
{
    int Max_GPU_ID = GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.count();
    QString MultiGPUSettings_str = "";
    for (int GPU_ID = 0; GPU_ID < Max_GPU_ID; GPU_ID++)
    {
        QMap<QString, QString> GPUInfo = GPUIDs_List_MultiGPU_RealCUGANNcnnVulkan.at(GPU_ID);
        if (GPUInfo["isEnabled"] != "true")
        {
            continue;
        }
...
RealCUGAN 块大小调整按钮
*/
void MainWindow::on_pushButton_Add_TileSize_RealCUGANNCNNVulkan_clicked()
{
    ui->spinBox_TileSize_RealCUGANNCNNVulkan->setValue(AddTileSize_NCNNVulkan_Converter(ui->spinBox_TileSize_RealCUGANNCNNVulkan->value()));
}

void MainWindow::on_pushButton_Minus_TileSize_RealCUGANNCNNVulkan_clicked()
{
    ui->spinBox_TileSize_RealCUGANNCNNVulkan->setValue(MinusTileSize_NCNNVulkan_Converter(ui->spinBox_TileSize_RealCUGANNCNNVulkan->value()));
}
