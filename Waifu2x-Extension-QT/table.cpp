...
int MainWindow::Table_FileCount_reload()
{
    ...
    if (filecount_total > 0)
    {
        ...
        ui->label_FileCount->setText(QString("File count: %1").arg(filecount_total));
        ui->label_FileCount->setToolTip(QString("Image: %1\nGIF: %2\nVideo: %3").arg(filecount_image).arg(filecount_gif).arg(filecount_video));
        //=================
        // 启用控制按钮
        //=================
...
int MainWindow::Table_Save_Current_Table_Filelist()
{
    ...
    //================= 添加警告 =========================
    configIniWrite->setValue("/Warning/EN", "Carefully Edit!!!");
    //================= 存储table_image =========================
...
void MainWindow::on_pushButton_SaveFileList_clicked()
{
    ...
    {
        QMessageBox::warning(this,"Error","File list is empty!");
        return;
    }
    ...
    emit Send_TextBrowser_NewMessage("Write to the file, please wait.");
...
int MainWindow::Table_Save_Current_Table_Filelist_Finished()
{
    ...
    emit Send_TextBrowser_NewMessage("File list saved successfully!");
    //===
    QMessageBox::warning(this,"Notification","File list saved successfully!");
    //===
    return 0;
}
int MainWindow::Table_Read_Saved_Table_Filelist()
{
    ...
    {
        emit Send_TextBrowser_NewMessage("Cannot find the saved Files List!");
    ...
    for (int i = 0; i < rowCount_image; i++)
    {
        ...
        if (status == "Failed")
        {
            emit Send_TextBrowser_NewMessage("[Failed]--[" + fullPath + "]");
        }
        //===============
        if (CustRes_str != "")
        {
...
int MainWindow::Table_Read_Saved_Table_Filelist_Finished()
{
    ...
    //============
    emit Send_TextBrowser_NewMessage("File list update is complete!");
    //====
    ...
    {
        emit Send_TextBrowser_NewMessage("The file list saved last time is empty.");
        progressbar_clear();
    }
    //==
