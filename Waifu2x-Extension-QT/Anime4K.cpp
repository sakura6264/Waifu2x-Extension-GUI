...
int MainWindow::Anime4k_Image(int rowNum, bool ReProcess_MissingAlphaChannel)
{
    ...
    QStringList args = {"-i", SourceFile_fullPath, "-o", OutPut_Path, "-z"};
    args.append(HDNDenoiseLevel_image);
    args.append(Anime4k_ReadSettings(PreserveAlphaChannel));
    //========
...
int MainWindow::Anime4k_GIF_scale(QMap<QString, QString> Sub_Thread_info, int *Sub_gif_ThreadNumRunning, bool *Frame_failed)
{
    ...
    //=======
    QStringList args = {"-i", InputPath, "-o", OutputPath, "-z", QString::number(ScaleRatio, 10)};
    args.append(HDNDenoiseLevel_gif);
    args.append(Anime4k_ReadSettings(false));
    //=======
    for (int retry = 0; retry < (ui->spinBox_retry->value() + ForceRetryCount); retry++)
    {
        QProcess *Waifu2x = new QProcess();
        ...
        if (isScaleComplete == false)
        {
            ...
            */
            ...
            for (int i = 0; i < Frame_fileName_list.size(); i++)
            {
                ...
                {
                    InterPro_now++;
                    emit Send_TextBrowser_NewMessage("File name:[" + SourceFile_fullPath + "]  Scale progress:[" + QString::number(InterPro_now, 10) + "/" + QString::number(InterPro_total, 10) + "] Duration progress:[" + QString::number(StartTime, 10) + "s/" + QString::number(VideoDuration, 10) + "s]");
                }
                int Sub_video_ThreadNumMax = ui->spinBox_ThreadNum_video_internal->value();
                if (waifu2x_STOP)
                {
...
int MainWindow::Anime4k_Video_scale(QMap<QString, QString> Sub_Thread_info, int *Sub_video_ThreadNumRunning, bool *Frame_failed)
{
    ...
    //=======
    QStringList args = {"-i", InputPath, "-o", OutputPath, "-z", QString::number(ScaleRatio, 10)};
    args.append(HDNDenoiseLevel_video);
    args.append(Anime4k_ReadSettings(false));
    //=======
    for (int retry = 0; retry < (ui->spinBox_retry->value() + ForceRetryCount); retry++)
    {
...
预读取引擎配置
*/
QStringList MainWindow::Anime4k_PreLoad_Settings()
{
    QStringList Anime4k_Settings_str;
    // 快速模式
    ...
    if (ui->checkBox_GPUMode_Anime4K->isChecked())
    {
        ...
        Anime4k_Settings_str.append("-M");
        Anime4k_Settings_str.append(GPGPU_Model);
        // 加载OpenCL选项
        if (GPGPU_Model == "opencl")
        {
            if (ui->checkBox_OpenCLParallelIO_A4k->isChecked())
                Anime4k_Settings_str.append("-P");
            Anime4k_Settings_str.append("-Q");
            Anime4k_Settings_str.append(QString::number(ui->spinBox_OpenCLCommandQueues_A4k->value(), 10));
        }
    }
    // ACNet
    ...
    // Passes
    Anime4k_Settings_str.append("-p");
    Anime4k_Settings_str.append(QString::number(ui->spinBox_Passes_Anime4K->value(), 10));
    // Push color count
    Anime4k_Settings_str.append("-n");
    Anime4k_Settings_str.append(QString::number(ui->spinBox_PushColorCount_Anime4K->value(), 10));
    // Push color strength
    Anime4k_Settings_str.append("-c");
    Anime4k_Settings_str.append(QString::number(ui->doubleSpinBox_PushColorStrength_Anime4K->value(), 'f', 2));
    // Push gradient strength
    Anime4k_Settings_str.append("-g");
    Anime4k_Settings_str.append(QString::number(ui->doubleSpinBox_PushGradientStrength_Anime4K->value(), 'f', 2));
    // Pre-processing
    if (ui->checkBox_EnablePreProcessing_Anime4k->isChecked())
    {
        ...
        {
            Anime4k_Settings_str.append("-r");
            Anime4k_Settings_str.append(QString::number(PreProcessingFilters, 10));
        }
        else
        {
            Anime4k_Settings_str.append("-r");
            Anime4k_Settings_str.append("4");
        }
    }
    // Post-processing
    if (ui->checkBox_EnablePostProcessing_Anime4k->isChecked())
    {
        ...
        {
            Anime4k_Settings_str.append("-e");
            Anime4k_Settings_str.append(QString::number(PostProcessingFilters, 10));
        }
        else
        {
            Anime4k_Settings_str.append("-e");
...
读取配置生成配置string
*/
QStringList MainWindow::Anime4k_ReadSettings(bool PreserveAlphaChannel)
{
    QStringList Anime4k_Settings_str;
    //=================================
...
QStringList MainWindow::Anime4k_GetGPUInfo()
{
    GetGPUInfo_QMutex_Anime4k.lock();
    //====
    QStringList GPU_List = ui->lineEdit_GPUs_Anime4k->text().simplified().remove(QRegExp("\\s")).split(":");
    ...
    {
        GPUInfo.append("-h");
        GPUInfo.append(PID_DID.at(0).trimmed());
        GPUInfo.append("-d");
        GPUInfo.append(PID_DID.at(1).trimmed());
    }
    else
    {
        GPUInfo.append("-h");
...
{
    QString text = cmdGetText(Anime4k_ProgramPath, {"-l"});
    QtConcurrent::run(this, &MainWindow::showTextExt, QString("GPU List for Anime4K"), text);
}

void MainWindow::on_lineEdit_GPUs_Anime4k_editingFinished()
{
    QString Format_str = "0123456789:,";
    QString GPUs_str = ui->lineEdit_GPUs_Anime4k->text().simplified().remove(QRegExp("\\s"));
...
void MainWindow::on_pushButton_VerifyGPUsConfig_Anime4k_clicked()
{
    ...
    for (int i = 0; i < GPU_List.count(); i++)
    {
        ...
        if (PID_DID.count() == 2)
        {
            VerRes.append("GPU [" + QString::number(i, 10) + "]: Platform ID:[" + PID_DID.at(0).trimmed() + "]" + " Device ID:[" + PID_DID.at(1).trimmed() + "]\n\n");
        }
        else
        {
            VerRes.append("Error\n\n");
    ...
    QtConcurrent::run(this,&MainWindow::showTextExt,QString("GPUs List"),VerRes);
    //======
...
void MainWindow::DenoiseLevelSpinboxSetting_Anime4k()
{
    if (ui->checkBox_ACNet_Anime4K->isChecked() && ui->checkBox_HDNMode_Anime4k->isChecked())
    {
        if (ui->comboBox_Engine_Image->currentIndex() == 3)
        {
            ...
            ui->spinBox_DenoiseLevel_image->setToolTip("Range:1~3");
            ui->label_ImageDenoiseLevel->setToolTip("Range:1~3");
        }
        if (ui->comboBox_Engine_GIF->currentIndex() == 3)
        {
            ...
            ui->spinBox_DenoiseLevel_gif->setToolTip("Range:1~3");
            ui->label_GIFDenoiseLevel->setToolTip("Range:1~3");
        }
        if (ui->comboBox_Engine_Video->currentIndex() == 2)
        {
            ...
            ui->spinBox_DenoiseLevel_video->setToolTip("Range:1~3");
            ui->label_VideoDenoiseLevel->setToolTip("Range:1~3");
        }
    }
    else
    {
        if (ui->comboBox_Engine_Image->currentIndex() == 3)
        {
            ...
            ui->spinBox_DenoiseLevel_image->setToolTip("Due to current settings, you can\'t adjust denoise level.");
            ui->label_ImageDenoiseLevel->setToolTip("Due to current settings, you can\'t adjust denoise level.");
        }
        if (ui->comboBox_Engine_GIF->currentIndex() == 3)
        {
            ...
            ui->spinBox_DenoiseLevel_gif->setToolTip("Due to current settings, you can\'t adjust denoise level.");
            ui->label_GIFDenoiseLevel->setToolTip("Due to current settings, you can\'t adjust denoise level.");
        }
        if (ui->comboBox_Engine_Video->currentIndex() == 2)
        {
            ...
            ui->spinBox_DenoiseLevel_video->setToolTip("Due to current settings, you can\'t adjust denoise level.");
            ui->label_VideoDenoiseLevel->setToolTip("Due to current settings, you can\'t adjust denoise level.");
        }
    }
}
